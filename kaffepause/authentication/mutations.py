...
from kaffepause.authentication.backend import JSONWebTokenBackend, Neo4jSessionBackend, Neo4jSocialBackend
...
class SocialAuthJWT(graphene.Mutation):
    ...
    def mutate(self, info, provider, access_token):
        ...
        Mutation to obtain a JSON Web Token
        """
        logger.debug("Authenticating user with provider %s and access token %s", provider, access_token)

        user = Neo4jSocialBackend().authenticate(provider=provider, access_token=access_token)
