...
class RelationshipStatusAdminForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if (
            self.cleaned_data["from_slug"] == self.cleaned_data["to_slug"]
            or self.cleaned_data["to_slug"] == self.cleaned_data["symmetrical_slug"]
            or self.cleaned_data["symmetrical_slug"] == self.cleaned_data["from_slug"]
        ):
            raise forms.ValidationError(
                "from, to, and symmetrical slugs must be different"
            )
